...
<body>
    ...
    <div class="container my-5">
        ...
        $sortColumn = isset($_GET['sort']) ? $_GET['sort'] : 'group_id'; //change this in different pages 
        ...
        if (isset($_GET['keyword'])) {
            ...
            $count_sql = "  SELECT COUNT(*) AS total FROM 
                                ...
                                LEFT OUTER JOIN 
                                    ...
                                    ON study_group.teacher_id = subquery.teacher_id
                                WHERE 
                                    group_id LIKE '%$keyword%' 
                                    OR group_name LIKE '%$keyword%' 
                                    OR teacher_name LIKE '%$keyword%'
                                ) AS subquery;
                            ";
            $search_sql = " SELECT 
                                study_group.group_id, 
            ...
            //change these sql commands in different page
            $count_sql = "SELECT COUNT(*) AS total FROM study_group";
            $search_sql = " SELECT 
                                study_group.group_id, 
                                study_group.group_name, 
                                subquery.teacher_name
        ...
        echo '<table class="table table-striped table-hover table-bordered">
                <thead>
                    <tr>
                        ...
                        <th scope="col"><a href="?page=' . $page . '&sort=group_id&order=' . ($sortColumn == 'group_id' && $sortOrder == 'ASC' ? 'DESC' : 'ASC') . '&keyword=' . $keyword . '">รหัสกลุ่ม</a></th>
                        <th scope="col"><a href="?page=' . $page . '&sort=group_name&order=' . ($sortColumn == 'group_name' && $sortOrder == 'ASC' ? 'DESC' : 'ASC') . '&keyword=' . $keyword . '">ชื่อเต็มกลุ่ม</a></th>
                        <th scope="col"><a href="?page=' . $page . '&sort=teacher_name&order=' . ($sortColumn == 'teacher_name' && $sortOrder == 'ASC' ? 'DESC' : 'ASC') . '&keyword=' . $keyword . '">ครูที่ปรึกษา</a></th>
        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            echo '<tr>
                    ...
                    <td class="d-flex justify-content-center"><button type="submit" class="btn btn-warning disabled"><i class="bi bi-pen-fill"></i></button></td>
