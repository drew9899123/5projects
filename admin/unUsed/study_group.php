...
if (isset($_POST['groupIds'])) {
    ...
    $deleteQuery = "DELETE FROM your_table_name WHERE group_id IN (" . implode(',', $groupIds) . ")";
    $result = mysqli_query($connection, $deleteQuery);

    if ($result) {
        echo "Selected rows deleted successfully.";
    } else {
        echo "Error deleting rows: " . mysqli_error($connection);
    }
}
...
if (isset($_GET['keyword'])) {
    ...
    $sql = "SELECT study_group.group_id, study_group.group_name, subquery.teacher_name
            FROM study_group
            LEFT OUTER JOIN (
                SELECT teacher_id, CONCAT(name,' ',surname) as teacher_name FROM teacher
            ) as subquery ON study_group.teacher_id = subquery.teacher_id
            WHERE group_id LIKE '%$keyword%' OR group_name LIKE '%$keyword%' OR teacher_name LIKE '%$keyword%';";
    $result = $con->query($sql);
    ...
    $sql = "SELECT COUNT(*) AS total FROM study_group";
    $result_count = $con->query($sql);
    $row_count = mysqli_fetch_array($result_count);
    $count = $row_count['total'];
}
